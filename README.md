...
| law | `courtlistener` (the newest opinions from the Atom feeds: all courts plus each court's own feed in rotation, or one court), `courtlistener-oral-arguments` (the podcast, with the MP3), `courtlistener-api` (dockets, judges and financial disclosures, one request a run inside the free tier's 125 a day), `courtlistener-catalog` (every court, judge with education and party, disclosure with what it reports, oral argument, opinion cluster with its reporter citations as tags, and the FJC database, from the quarterly bulk dumps; dockets opt-in) | `COURTLISTENER_TOKEN` for the API sources only; the feeds and the dumps are keyless and public domain |
...
| profiles | `openprofiles` (p0dcasters and OutreachGraph listings first; any app that lists the OpenProfile.md files it serves), `sportarr-persons` (Sportarr's 111k names, kept only when Wikidata knows the person as a human with a sport, with their socials), `sportsdb-players`, `installmap-yc-people` (YC companies that run an MCP server, as organizations with their accounts, and their founders) | no |
...
| mcp | `mcp-registry` (the official registry: one row per server at its newest version, with packages, remotes and status), `docker-mcp` (Docker's catalogue: pull counts, categories, licences and the tools each server exposes), `smithery` (how often each server is actually called through Smithery's proxy), `awesome-mcp` (the punkpeye, wong2 and appcypher lists plus the protocol repo's reference servers), `aitmpl-mcps` (ready-made configs), `github-mcp-topics` (repositories tagged for MCP, newest push first), `installmap-yc-mcp` (Y Combinator companies that run a server, from Installmap's CC BY crawl, as OpenMCP records with the endpoint probed, plus each company's accounts and founders; the companies and founders also go to `profiles` as OpenProfiles through `installmap-yc-people`). Deduplicated on URL: the first source to carry a server keeps it. [docs/mcp-and-workflows.md](docs/mcp-and-workflows.md) | keyless (`GITHUB_TOKEN` raises the search and enrichment rate limits) |
| workflows | `reddit-workflows` (the r/ClaudeWorkflows library, one post per workflow with its rating, freshness, level and categories parsed out of the post), `awesome-claude-code` (read from the CSV the list is generated from, so entries its maintainer has marked dead are skipped), `aitmpl-components` (settings bundles, agent loops and behaviour mods), `github-agent-topics` (repositories tagged for Claude Code, skills and subagents). Served at `/workflows` as well as `/c/workflows`. [docs/mcp-and-workflows.md](docs/mcp-and-workflows.md) | keyless (`GITHUB_TOKEN` optional, as above) |
| skills | `aitmpl-skills` (890 skills with install counts and the command that installs each), `awesome-claude-code-skills` (the hand-picked ones). `/skills` | no |
| agents | `aitmpl-agents` (420 subagents by category), `awesome-agents` (the community lists that name each subagent rather than the bundle it ships in; VoltAgent seeded). `/agents` | no |
| commands | `aitmpl-commands` (290 slash commands with install counts). `/commands` | no |
| plugins | `plugin-marketplaces` (every plugin declared in the `.claude-plugin/marketplace.json` of the repos you name, the official one included). `/plugins` | no |
| hooks | `aitmpl-hooks` (what runs before and after a tool call, a prompt or a session). `/hooks` | no |
...
Every label in IANA's root, read daily and diffed into a change log nobody else publishes, with each registrar's one-year price to register, renew, transfer and restore beside it: Porkbun, Dynadot, Cloudflare and OVHcloud today, and any registrar that serves an [OpenTLD](https://logicsrc.com/docs/opentld) file. Faceted by type, renewal band, registrar, script, status and registry, sortable on every column, renewal first, with the renewal trap (a second year at twice the first or more) as a filter of its own. `/tlds/check` asks each registry over RDAP whether a name is taken, and never calls a 404 "available". The same rows are `GET /api/v1/tlds` (JSON or CSV), MCP (`search_tlds`, `get_tld`, `tld_changes`, `check_domain`) and the CLI (`nichedb tlds`, `nichedb check`). See [docs/tlds.md](docs/tlds.md).
...
| `github-repo` | stars, forks, topics, licence, language, last push | packages, extensions, mcp, workflows, skills, agents, commands, plugins, hooks |
