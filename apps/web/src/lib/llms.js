...
export async function llmsTxt() {
  ...
  const lines = [
    ...
    '## Population',
    '',
    'Who lives where, drilled down from the world to the ZIP code. Countries from the World',
    'Bank, US states, cities and ZIP codes from the Census ACS 5-year estimates, the cities of',
    'every other country from GeoNames. Keys are us, us-ca, us-ca-los-angeles, us-90210, gb,',
    'gb-england-london.',
    '',
    `- \`GET ${base}/api/v1/population\` — every country, largest first, and counts per level`,
    `- \`GET ${base}/api/v1/population/{key}\` — one area, what it contains, its largest children`,
    `- \`GET ${base}/api/v1/population/areas?within={key}&level=country|state|city|zip\` — page a level`,
    `- \`GET ${base}/api/v1/population/zip/{zip}\`, \`/search?q={name}\` — find an area`,
    '',
    '## Vehicles',
    '',
    ...
    '## Top-level domains',
    '',
    "Every label in IANA's root, diffed daily, with register, renewal, transfer and restore",
    'prices from several registrars, and an RDAP lookup. The record and the price shape are',
    'OpenTLD (https://logicsrc.com/docs/opentld).',
    '',
    `- \`GET ${base}/api/v1/tlds\` — filter and sort: \`q\`, \`type\`, \`manager\`, \`registrar\`, \`band\`,`,
    '  `max_renew`, `max_register`, `trap=1` (renews at 2x or more), `idn`, `status`, `sort`',
    '  (renew, register, transfer, ratio, tld, registrars, type, manager, first_seen), `order`,',
    '  `limit`, `offset`, `format=csv`. Answers carry facet counts. "Best" is the cheapest USD',
    "  price; with `registrar` it is that registrar's price in its own currency.",
    `- \`GET ${base}/api/v1/tlds/{tld}\` — one label, every registrar's price, its change history`,
    `- \`GET ${base}/api/v1/tlds/changes\` — labels added to and removed from the root, newest first`,
    `- \`GET ${base}/api/v1/tlds/check?name=foo&tlds=com,dev\` — RDAP: registered, not_registered or`,
    `  unknown. not_registered is not "for sale". ${config.tlds.freeChecksPerHour} names an hour free.`,
    '',
    '## AI incidents',
    '',
    ...
    'item and a tracker. Three paid ways in:',
    '',
    `- **Premium, $${(config.premium.dayCents / 100).toFixed(2)} a day** (${base}/premium), or $${(config.premium.monthCents / 100).toFixed(0)} a month`,
    `  or $${(config.premium.yearCents / 100).toFixed(0)} a year: no ads, no tracking, ${config.api.premiumPerHour.toLocaleString('en-US')} API requests an hour,`,
    ...
    `- **Data, $${(config.dataDumps.priceCents / 100).toLocaleString('en-US')} per 30 days** (${base}/dumps): everything in Pro plus`,
    `  full public-data snapshots every hour, as gzip-compressed NDJSON. Authenticated manifest: ${base}/api/v1/dumps/latest.`,
    `  Completed dumps are retained ${config.dataDumps.retentionHours} hours. No automatic renewal.`,
