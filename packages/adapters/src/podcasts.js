...
export const podcasts = defineAdapter({
  ...
  configFields: [
    ...
    },
    {
      ...
      label: 'Pages per run',
      type: 'number',
      help: `Pages of ${PAGE} feeds to read before giving up on reaching last run's marker. Raise it after a bulk import, when the directory is classifying faster than an hour's worth.`,
    },
  ],
  defaults: { group: 'self-hosted', maxPages: 10 },
  defaultSources: [
    {
      slug: GROUPS.commercial.slug,
      name: GROUPS.commercial.name,
      description: GROUPS.commercial.description,
      config: { group: 'commercial', maxPages: 10 },
    },
    {
      slug: GROUPS['self-hosted'].slug,
      name: GROUPS['self-hosted'].name,
      description: GROUPS['self-hosted'].description,
      config: { group: 'self-hosted', maxPages: 10 },
    },
  ],
  async pull({ config, cursor, http, log, deadline }) {
    ...
    return { items, cursor: newest ? { newest } : (cursor ?? {}) };
  },
});
